...
export function createSkillRegistryController() {
  ...
  const controller: SkillRegistryController = {
    ...
    // Skills are stored by toolName (hyphens normalized to underscores), but agents
    // naturally refer to skills by their directory name, which keeps hyphens. Fall
    // back to the normalized form so either spelling resolves to the same skill.
    get: (key) => store.get(key) ?? store.get(key.replace(/-/g, '_')),
    set: (key, skill) => {
      store.set(key, skill);
    },
  };
